...
from Musikku.utils.formatters import formats
from Musikku.utils.inline.play import (livestream_markup,
                                          playlist_markup,
...
@app.on_message(
        filters.command(["PLAY_COMMAND"]) | filters.command(["تشغيل","شغل","ش"],prefixes= ["/", "!",""])
    & filters.group
    & ~filters.edited
    & ~BANNED_USERS
)
...
@app.on_callback_query(
    filters.regex("MusikkuPlaylists") & ~BANNED_USERS
)
@languageCB
